// apps/backend/src/routes/vendorServices.ts
...
/**
 * GET /api/vendor-services/:ownerId
 * List all service mappings for a vendor.
 */
...
/**
 * POST /api/vendor-services/:ownerId
 * Create or upsert a mapping for a given service kind.
 * Frontend calls this when enabling a service the first time.
 */
router.post("/:ownerId", requireAuth, async (req: any, res, next) => {
  try {
    if (!isHrmsAdmin(req.user)) {
      return res.status(403).json({
        error: "Only HR / Admin / Super Admin can modify vendor services",
      });
    }
...
/**
 * PATCH /api/vendor-services/:ownerId/:id
 * Update an existing mapping (usually just toggling enabled).
 */
router.patch("/:ownerId/:id", requireAuth, async (req: any, res, next) => {
  try {
    ...
    if (!doc) {
      return res.status(404).json({ error: "Vendor service mapping not found" });
    }
