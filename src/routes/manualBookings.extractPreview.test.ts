...
describe("POST /extract-preview", () => {
  ...
  it("a hotel voucher → extracted:true, docType hotel, flight:null (Step 3 territory)", async () => {
    extractMock.mockResolvedValue({ docType: "hotel", voucher: { type: "hotel" }, rawCandidate: null, modelUsed: "m", validationErrorCount: 0 });
    const r = await post();
    expect(r.status).toBe(200);
