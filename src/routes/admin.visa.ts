...
/* ── THE UNIFIED MASTER SHEET LIVES NEXT DOOR ─────────────────────────
 * GET /master-sheet/people and /master-sheet/people/export were defined on
 * THIS router when they shipped (Deploy 2), which put two owners on the
 * /api/admin/visa/master-sheet prefix: this file and
 * routes/admin.visa.masterSheet.ts, which the server already mounts AT that
 * prefix. Two routers answering under one prefix is a shadowing bug waiting
 * for the day somebody adds a path that exists in both — and it is answered
 * by whichever router server.ts happens to mount first, which is not a fact
 * anybody should have to know to read a route.
 *
 * The routes MOVED to admin.visa.masterSheet.ts, unchanged. The URLs are
 * byte-identical (that router mounts at /api/admin/visa/master-sheet, so
 * "/master-sheet/people" here became "/people" there); only the owning file
 * changed. Nothing new may be added to this router under /master-sheet.
 */
