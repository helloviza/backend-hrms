...
async function safeAdvanceStage(
  ...
  const reqDoc: any = await ApprovalRequest.findById(requestId);
  if (!reqDoc) return;
...
const requireProposalViewer: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const p: any = await Proposal.findById(proposalId).lean();
    if (!p) return res.status(404).json({ error: "Proposal not found" });
    if (!p.requestId) return res.status(400).json({ error: "Proposal missing requestId" });

    const ar: any = await ApprovalRequest.findById(p.requestId).lean();
...
router.get("/email-action", async (req: Request, res: Response) => {
  try {
    ...
    let ar: any = null;
    if (doc.requestId) ar = await ApprovalRequest.findById(doc.requestId).lean();
    ...
    if (shouldDecline) {
      doc.status = "DECLINED";
      await doc.save();
      if (doc.requestId) await safeAdvanceStage(doc.requestId, "PROPOSAL_DECLINED");
    ...
    if (fullyApproved) {
      doc.status = "APPROVED";
      await doc.save();
      if (doc.requestId) await safeAdvanceStage(doc.requestId, "PROPOSAL_APPROVED");
...
router.post("/by-request/:requestId/draft", requireAnyAuth, requireStaff, requireTravelMode("APPROVAL_FLOW"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const ar: any = await ApprovalRequest.findById(rid).lean();
    ...
    if (latest) {
      ...
      await doc.save();
      await safeAdvanceStage(rid, "PROPOSAL_PENDING");
...
router.put("/:id", requireAnyAuth, requireStaff, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const doc: any = await Proposal.findById(id);
...
router.post("/:id/submit", requireAnyAuth, requireStaff, requireTravelMode("APPROVAL_FLOW"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await doc.save();
    if (doc.requestId) await safeAdvanceStage(doc.requestId, "PROPOSAL_SUBMITTED");
