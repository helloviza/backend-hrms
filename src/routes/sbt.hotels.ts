...
async function fetchCityList(countryCode: string): Promise<CityEntry[]> {
  ...
  const res = await fetch(
    `http://api.tbotechnology.in/TBOHolidays_HotelAPI/CityList?CountryCode=${countryCode}`,
    {
      method: "POST",
      headers: {
...
router.post("/search", requireSBT, requireHotelAccess, async (req: any, res: any) => {
  try {
    ...
    for (let i = 0; i < chunks.length; i += MAX_CONCURRENT) {
      ...
      const promises = batch.map((hotelCodeChunk) =>
        fetch("https://affiliate.tektravels.com/HotelAPI/Search", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
router.post("/book", requireSBT, requireHotelAccess, async (req: any, res: any) => {
  try {
    ...
    const HotelPassenger = (Guests || []).map((g: any) => ({
      Title: g.Title || "Mr",
...
// ─── 10. GET /details?hotelCodes=xxx,yyy ─────────────────────────────────────
...
router.get("/details", async (req: any, res: any) => {
  try {
    ...
    const tboRes = await fetch(
      "https://apiwr.tboholidays.com/HotelAPI/Hoteldetails",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
