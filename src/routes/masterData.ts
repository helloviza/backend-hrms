...
router.get("/", requireAuth, async (req, res, next) => {
  try {
    ...
    const filter: any = { status: /approved/i };

    if (type && type !== "All") filter.type = new RegExp(`^${type}$`, "i");
    if (status && /inactive/i.test(status)) filter.isActive = false;
    if (status && /active/i.test(status)) filter.isActive = true;
...
router.post(
  ...
  if (!(onboardingDoc as any).welcomeEmailSent) {
    await sendOnboardingWelcomeEmail({
      to: email,
      counterpartyName: baseName || "Employee",
      effectiveDate:
