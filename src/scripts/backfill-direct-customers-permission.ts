...
async function main() {
  ...
  for (const doc of docs) {
    ...
    const current = modules?.directCustomers?.access ?? "NONE";

    // Idempotent: skip if already explicitly set to something other than NONE
    if (current !== "NONE") {
      countSkipped++;
      continue;
    }
    ...
    const hasManualBookingsWrite = modules?.manualBookings?.access === "WRITE";

    if (hasFullAll) {
      await UserPermission.updateOne(
        { _id: doc._id },
        { $set: { "modules.directCustomers": { access: "FULL", scope: "ALL" } } },
      );
      countFullAll++;
    } else if (hasFullWs) {
      await UserPermission.updateOne(
        { _id: doc._id },
        { $set: { "modules.directCustomers": { access: "FULL", scope: "WORKSPACE" } } },
      );
      countFullWs++;
    } else if (hasManualBookingsWrite) {
      await UserPermission.updateOne(
        { _id: doc._id },
        { $set: { "modules.directCustomers": { access: "WRITE", scope: "OWN" } } },
      );
      countWriteOwn++;
    } else {
      countSkipped++;
