/**
 * Cases 5–8: International hotel certification
 * Run: cd apps/backend && npx tsx src/scripts/test-hotel-intl.ts <5|6|7|8>
 */
...
const INTL_HOTEL_CODE = "1000171";
const INTL_CITY_LABEL = "Dubai (intl sandbox)";

const INTL_SEARCH_CODES = [
  "1000084", "1000027", "1000089", "1000171", "1000167",
  "1000302", "1000198", "1000538", "1000648", "1000937",
  "1000957", "1000847", "1000872", "1001049", "1001056",
  "1001143", "1001122", "1001046", "1463986", "1491912",
].join(",");

const PASSPORT_NOS = [
  "Z1234567", "Z7654321", "Z1112233", "Z4455667",
  "Z5566778", "Z6677889",
];
...
const caseConfigs: Record<number, PaxRoom[]> = {
  5: [{ Adults: 1, Children: 0, ChildrenAges: null }],
  6: [{ Adults: 2, Children: 2, ChildrenAges: [8, 6] }],
  7: [
...
const caseLabelMap: Record<number, string> = {
  5: "Case5_Hotel_Intl_1R_1A",
  6: "Case6_Hotel_Intl_1R_2A2C",
  7: "Case7_Hotel_Intl_2R_1A_1A",
  8: "Case8_Hotel_Intl_2R_1A2C_2A",
};
  ...
  return paxRooms.map((room, roomIdx) => {
    ...
    for (let a = 0; a < room.Adults; a++) {
      ...
      passengers.push({
        ...
        Age: 0,
        PassportNo: passportMandatory ? PASSPORT_NOS[paxIndex] : "",
        PassportIssueDate: passportMandatory ? "2020-01-15T00:00:00" : "0001-01-01T00:00:00",
        PassportExpDate: passportMandatory ? "2030-01-15T00:00:00" : "0001-01-01T00:00:00",
...
async function main() {
  const caseNum = parseInt(process.argv[2] || "5", 10);
  if (![5, 6, 7, 8].includes(caseNum)) {
    console.error("Usage: npx tsx src/scripts/test-hotel-intl.ts <5|6|7|8>");
    process.exit(1);
  }

  console.log(`\n=== TBO Hotel Certification — Case ${caseNum} (International) ===\n`);

  const paxRooms = caseConfigs[caseNum];
  const { checkIn, checkOut } = getFutureDates();
  const caseTraceId = `hotel-intl-case${caseNum}-${randomUUID().slice(0, 8)}`;
  ...
  console.log(`[HOTEL-CODES] Using hardcoded international hotel code: ${INTL_HOTEL_CODE} (confirmed working in Case 5)`);
  ...
  console.log(`[PREBOOK] PassportMandatory: ${passportMandatory}`);
  ...
  const hotelRoomsDetails = buildPassengers(paxRooms, panMandatory, passportMandatory);
...
╠══════════════════════════════════════════════════╣
  ...
  City:           ${INTL_CITY_LABEL}
  Hotel Code:     ${selectedHotel.HotelCode}
  CheckIn:        ${checkIn}
  CheckOut:       ${checkOut}
